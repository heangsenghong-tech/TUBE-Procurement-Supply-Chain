// Request Center: purchase requests (incl. petty cash), sample requests, approvals, tracking.
import { and, asc, desc, eq, ilike, inArray, or, sql, type SQL } from 'drizzle-orm';
import {
  type ApprovalActionInput, type PurchaseRequestInput, type SampleRequestInput, type SampleEvaluationStatus
} from '@tube/shared';
...
async function priceLines(db: DbOrTx, lines: PurchaseRequestInput['lines']) {
  ...
  return { priced, total: sum(priced.map((p) => p.amount)) };
}
...
export async function createPurchaseRequest(db: Db, actor: Actor, input: PurchaseRequestInput) {
  ...
  const { priced, total } = await priceLines(db, input.lines);
  const { rule, steps } = await pickRule(db, 'request', total, { track: input.track });

  return db.transaction(async (tx) => {
    const number = await nextNumber(tx, 'PR');
    const [req] = await tx.insert(t.requests).values({
      number, kind: 'purchase', track: input.track, orgUnitId: unit.id, requesterId: actor.id,
    ...
    await audit(tx, { userId: actor.id, action: 'request.create', entityType: 'request', entityId: req!.id,
      after: { number, total, rule: rule.name, pettyCash: rule.isPettyCash, lines: priced.length } });
    return { id: req!.id, number, isPettyCash: rule.isPettyCash };
  });
}
...
export async function resubmitPurchaseRequest(db: Db, actor: Actor, requestId: string, input: PurchaseRequestInput) {
  ...
  if (existing.kind !== 'purchase') throw badRequest('Only purchase requests can be resubmitted.');
  ...
  const { priced, total } = await priceLines(db, input.lines);
  const { rule, steps } = await pickRule(db, 'request', total, { track: existing.track });
  ...
  return db.transaction(async (tx) => {
    ...
    await audit(tx, { userId: actor.id, action: 'request.resubmit', entityType: 'request', entityId: req.id,
      before: { total: req.estimatedTotal, lines: old.filter((l) => l.status === 'pending_approval').map((l) => ({ itemId: l.itemId, qty: l.qty })) },
      after: { total, rule: rule.name, lines: priced.map((p) => ({ itemId: p.itemId, qty: p.qty })) } });
...
export async function cancelRequest(db: Db, actor: Actor, requestId: string, reason: string) {
  return db.transaction(async (tx) => {
    ...
    const cancellable = req.kind === 'sample'
      ? !['pass', 'fail', 'not_meet_requirement', 'cancelled'].includes(req.status)
      : ['pending_approval', 'changes_requested', 'approved'].includes(req.status);
    if (!cancellable) throw conflict('This request can no longer be cancelled here. If items are already being sourced, ask Procurement to cancel the lines.');
...
export async function createSampleRequest(db: Db, actor: Actor, input: SampleRequestInput) {
  ...
  return db.transaction(async (tx) => {
    ...
    const [req] = await tx.insert(t.requests).values({
      number, kind: 'sample', track: input.track, orgUnitId: unit.id, requesterId: actor.id, status: 'sourcing',
...
export async function listRequests(db: DbOrTx, actor: Actor, f: RequestListFilters = {}) {
  ...
  const rows = await db.select({
    r: t.requests, unitName: t.orgUnits.name, requesterName: t.users.name,
  ...
  return rows.map(({ r, unitName, requesterName, lineCount }) => ({
...
export async function getRequest(db: DbOrTx, actor: Actor, id: string) {
  ...
  return {
    ...
    permissions: {
      canAct,
      canCancel: (r.requesterId === actor.id || can(actor, 'procurement.operate')) &&
        (r.kind === 'sample' ? !['pass', 'fail', 'not_meet_requirement', 'cancelled'].includes(r.status) : ['pending_approval', 'changes_requested', 'approved'].includes(r.status)),
...
export async function approvalInbox(db: DbOrTx, actor: Actor) {
  ...
  const out: { kind: 'request' | 'po'; documentId: string; number: string; title: string; amount: number; stepLabel: string; override: boolean; submittedAt: Date; by: string }[] = [];
  for (const { step, instance } of pending) {
    if (instance.documentKind === 'request') {
      ...
      const unit = await db.query.orgUnits.findFirst({ where: eq(t.orgUnits.id, r.orgUnitId) });
      const requester = await db.query.users.findFirst({ where: eq(t.users.id, r.requesterId) });
      out.push({ kind: 'request', documentId: r.id, number: r.number, title: `${unit?.name ?? ''}${r.isPettyCash ? ' · Petty cash' : ''}`,
        amount: r.estimatedTotal, stepLabel: step.actionLabel, override: how === 'override', submittedAt: r.submittedAt, by: requester?.name ?? '' });
    } else {
      ...
      out.push({ kind: 'po', documentId: po.id, number: po.number, title: supplier?.name ?? '', amount: po.total,
        stepLabel: step.actionLabel, override: how === 'override', submittedAt: po.createdAt, by: creator?.name ?? '' });
    }
  }
